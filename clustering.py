...
def stream_cluster(forward_index, radius):
    ...
    for doc, terms in forward_index_set.items():
        # Try to assign the document to an existing cluster
        assigned = False
        for medoid, cluster in clusters:
            if jaccard_distance(terms, forward_index_set[medoid]) <= radius:
                cluster.append(doc)
                assigned = True
                break
...
def forward_index(inverted_index):
    ...
    return dict(sorted(forward_index.items(), key=lambda item: len(item[1]), reverse=True))


def clustering(inverted_index, radius):
    '''Take an inverted index and radius.
    ...
    clusters = stream_cluster(f_i, radius)
